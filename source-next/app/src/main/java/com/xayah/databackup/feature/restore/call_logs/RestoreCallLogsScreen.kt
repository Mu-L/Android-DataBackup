...
import com.xayah.databackup.ui.component.selection.CallLogListItem
import com.xayah.databackup.util.Navigator
import com.xayah.databackup.util.popBackStackSafely

@Composable
fun RestoreCallLogsScreen(navigator: Navigator, viewModel: CallLogsViewModel) {
    val state by viewModel.state.collectAsStateWithLifecycle()
    val items by viewModel.items.collectAsStateWithLifecycle()
    val search by viewModel.searchText.collectAsStateWithLifecycle()
    RestoreListScaffold(
        title = stringResource(R.string.select_call_logs),
        ...
        CallLogListItem(
            ...
            callLog = item.value.copy(selected = item.key in state.selected),
